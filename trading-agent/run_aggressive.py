"""
Aggressive Strategy Runner
Runs the aggressive trading configuration
"""
import sys
import os
...
import config_aggressive as config_module
...
class AggressiveTradingRunner:
    """Runs aggressive trading strategy"""
    ...
    def morning_routine(self):
        print("\n" + "="*70)
        print(f"🌅 AGGRESSIVE - MORNING - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("="*70)
    ...
    def trading_routine(self):
        print("\n" + "="*70)
        print(f"📈 AGGRESSIVE - TRADING - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("="*70)
    ...
    def position_check(self):
        print(f"\n🔍 AGGRESSIVE - Position check - {datetime.now().strftime('%H:%M:%S')}")
        try:
    ...
    def end_of_day_routine(self):
        print("\n" + "="*70)
        print(f"🌙 AGGRESSIVE - END OF DAY - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("="*70)
    ...
    def run_continuously(self):
        print("\n🚀 Starting AGGRESSIVE continuous trading mode...")
        print("\nSchedule:")
...
def main():
    ...
    runner = AggressiveTradingRunner()
